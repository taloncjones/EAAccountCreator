...
class Browser:
			...
			while True:
				try:
					...
					if (iteration == MAX_RETRIES):
						raise ValueError("Maximum number of retries hit: {i}. Aborting execution".format(i=MAX_RETRIES))
					break
				return
	...
	def fillText(self, id, text):
		try:
			...
			self.showWindow()
			LOGGER.debug('Error: Could not find text field: ', id)
			exit(1)
	...
	def clickButton(self, id, lookupType='id'):
		...
		try:
			...
			self.showWindow()
			LOGGER.debug('Error: Could not find button with ', lookupType, ':', id)
			exit(1)
